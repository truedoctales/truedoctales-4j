 ...
 *
 * <p>Models the silent risks that grow in the gap between what is written and what is built. Each
 * risk has a severity and a structural mitigation.
 */
@Plot("Risk")
public class RiskPlot {
  ...
  public void createRisk(
      ...
      @Variable(
              value = "mitigation",
              description = "The structural solution that defeats this risk")
          String mitigation) {
    monsterService.createMonster(id, name, severity, mitigation);
  }

  @Step(
      value = "Risk is active",
      description = "Asserts that the risk is currently active in the project.")
  public void riskIsActive(@Variable(value = "name", description = "Risk name") String name) {
    Assertions.assertTrue(monsterService.isAlive(name), "Risk '" + name + "' should be active");
  }

  @Step(
      value = "Risk is mitigated",
      description = "Asserts that the risk has been resolved and is no longer active.")
  public void riskIsMitigated(@Variable(value = "name", description = "Risk name") String name) {
    Assertions.assertFalse(monsterService.isAlive(name), "Risk '" + name + "' should be mitigated");
  }
}
