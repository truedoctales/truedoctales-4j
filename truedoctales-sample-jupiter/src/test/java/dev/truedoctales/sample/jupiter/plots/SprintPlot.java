 ...
 *
 * <p>Tracks the critical gap between <em>reported</em> velocity (what the developer
 * said was done) and <em>verified</em> velocity (what QA confirmed actually works).
 *
 * <p>A sprint where reported velocity is high but verified velocity is low has not
 * delivered — it has generated the illusion of delivery.
 */
@Plot("Sprint")
public class SprintPlot {
  ...
  public void planSprint(
      ...
      @Variable(value = "plannedPoints", description = "Story points committed for this sprint") Integer plannedPoints,
  ...
  @Step(value = "Add task ${sprint}",
      description = "Adds one or more tasks to the sprint. Each task has a title and story point value.")
  public void addTask(
  ...
  @Step(value = "Mark task done",
      description = "Developer marks a task as done. Updates reported velocity. "
  ...
  public void markTaskDone(
    ...
    Assertions.assertTrue(sprintService.markTaskDone(task),
        "Task '" + task + "' must be registered in a sprint");
  }

  @Step(value = "Verify task",
  ...
  public void verifyTask(
    ...
    Assertions.assertTrue(sprintService.verifyTask(task),
        "Task '" + task + "' must be marked done before it can be verified");
  }

  @Step(value = "Task is not verified",
      description = "Asserts that the task has NOT been verified by QA. "
          + "A done-but-unverified task is a gap in the sprint.")
  public void taskIsNotVerified(
      @Variable(value = "task", description = "Task title") String task) {
    Assertions.assertFalse(sprintService.isTaskVerified(task),
        "Task '" + task + "' should not be verified yet");
  }

  @Step(value = "Close sprint",
      description = "Closes the sprint. Status is COMPLETED if all done tasks are verified, FAILED otherwise.")
  ...
  @Step(value = "Sprint status is",
      description = "Asserts the sprint's final status: COMPLETED or FAILED.")
  public void sprintStatusIs(
      @Variable(value = "sprint", description = "Sprint name") String sprint,
      @Variable(value = "expected", description = "Expected status: COMPLETED or FAILED") String expected) {
    Sprint s = sprintService.findByName(sprint)
        .orElseThrow(() -> new AssertionError("Sprint '" + sprint + "' must exist"));
    Assertions.assertEquals(expected, s.status().name(),
        "Sprint '" + sprint + "' should have status " + expected);
  }

  @Step(value = "Reported velocity is",
      description = "Asserts the developer-reported story points for the sprint. "
  ...
  public void reportedVelocityIs(
    ...
    Sprint s = sprintService.findByName(sprint)
        .orElseThrow(() -> new AssertionError("Sprint '" + sprint + "' must exist"));
    Assertions.assertEquals(expected, s.reportedPoints(),
        "Sprint '" + sprint + "' reported velocity should be " + expected);
  }

  @Step(value = "Verified velocity is",
      description = "Asserts the QA-verified story points for the sprint. "
  ...
  public void verifiedVelocityIs(
    ...
    Sprint s = sprintService.findByName(sprint)
        .orElseThrow(() -> new AssertionError("Sprint '" + sprint + "' must exist"));
    Assertions.assertEquals(expected, s.verifiedPoints(),
