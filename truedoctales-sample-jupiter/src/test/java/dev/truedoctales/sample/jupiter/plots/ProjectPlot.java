 ...
 *
 * <p>The key insight: a feature is only "live" if it has at least one verified specification
 * example. Documentation alone is not proof of existence.
 *
 * <p>Use {@code Feature not live} to expose the gap between the product catalogue and the actual
 * running system.
 */
@Plot("Project")
public class ProjectPlot {
  ...
  @Step(value = "Project exists", description = "Asserts that the project has been registered.")
  public void projectExists(@Variable(value = "name", description = "Project name") String name) {
    Assertions.assertTrue(projectService.exists(name), "Project '" + name + "' should exist");
  }

  @Step(
      value = "Feature is live",
      description =
          "Asserts that a feature has at least one verified specification example, "
  ...
  public void featureIsLive(
    ...
    Assertions.assertTrue(
        specificationService.hasExamples(feature),
        "Feature '"
            + feature
            + "' in project '"
            + project
            + "' has no verified examples. "
            + "It is in the catalogue — but it is not live.");
  }

  @Step(
      value = "Feature not live",
      description =
          "Asserts that a feature has NO verified specification examples. "
